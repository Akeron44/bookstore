...
pgClient.on("error", (e) => {
  getLogger().debug("Error occurred in Postgres client", e);
  throw e;
});

pgClient
  .query("CREATE TABLE IF NOT EXISTS books (title varchar(128))")
  .catch((err) => {
    getLogger().debug("Error occurred while creating table", err);
    throw err;
  });
...
const getCpuUtilizationPromise = () =>
  new Promise((resolve) => {
...
router.route("/books").get(async (_req, res) => {
  try {
...
router.route("/generate-cpu-load").post(async (req, res) => {
  try {
    res.sendStatus(202);
